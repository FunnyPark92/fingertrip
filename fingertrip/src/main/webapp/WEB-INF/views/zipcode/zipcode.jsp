...
<script type="text/javascript">
	...
	function makeList(res) {
		...
		if (totalCount > 0) {
			...
			$(res).find("juso").each(function(idx, item){
				...
				result += "<td><a href='#'>" + $(item).find("roadAddr").text() + "</a></td>";
				result += "</tr>";
			});
			result += "</table>";
			$("#divList").html(result);
	...
	$.pagingProc=function() {
		//페이징 처리
		var blockSize = 10;
		pagination($("#currentPage").val(), $("#countPerPage").val(), blockSize, totalCount);
...
<body>
	...
	<form name="frmZip" id="frmZip" method="post" action="<c:url value='/zipcode/zipcode.do'/>">
